## AnimationTree experiment: agents use TreeAgent (AnimationTree-based) instead of
## DemoAgent (AnimationPlayer-based). Tests whether SpringResolver correctly reads
## blended poses from AnimationTree, additive flinch overlays, and OneShot recovery.
## Controls:
...
func _ready() -> void:
	...
	_hit_label.text = "TREE AGENT | LMB = shoot | 1-5 = weapon | K = kill | R = revive"
...
func _kill_nearest() -> void:
	...
	for kc: KickbackCharacter in _characters:
		...
		if not root or kc.get_active_state_name() == "PERSISTENT":
			continue
...
func _process(_delta: float) -> void:
	...
	for i in _characters.size():
		...
		# Show TreeAgent animation state
		var tree_info := ""
		if root:
			var agent := root.get_node_or_null("TreeAgent") as TreeAgent
			if not agent:
				agent = root.get_node_or_null("PatrolAgent") as TreeAgent
			if agent:
				tree_info = "/%s" % agent.get_tree_state()

		info_parts.append("A%d:%s(%s%s,%.0fm)" % [
			i + 1, kc.get_tier_name(), kc.get_active_state_name(), tree_info, dist])

	_state_label.text = "TREE [%d] %s | FPS: %d | Active: %d | %s" % [
