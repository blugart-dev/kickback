...
func _unhandled_input(event: InputEvent) -> void:
	if event is InputEventKey and event.pressed:
		...
		if prev != _current_profile_idx:
			_hit_label.text = "Switched to: %s" % _profiles[_current_profile_idx].profile_name
...
func _shoot(screen_pos: Vector2) -> void:
	...
	if collider is RigidBody3D:
		var profile: ImpactProfile = _profiles[_current_profile_idx]
...
func _process(_delta: float) -> void:
	var profile: ImpactProfile = _profiles[_current_profile_idx]
	var wname: String = profile.profile_name if profile else "NONE"
