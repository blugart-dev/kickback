...
func _shoot(screen_pos: Vector2) -> void:
	...
	if char_root:
		...
		if kc:
			...
			_hit_label.text = "Hit: %s on %s with %s" % [collider.name, char_root.name, profile.profile_name]
			return
...
func _process(_delta: float) -> void:
	...
	var info := "[%d] %s  |  FPS: %d  |  Frame: %.1fms  |  Active ragdolls: %d  |  RMB+WASD = fly  |  1-5 = weapon\n" % [
		_current_weapon + 1, profile.profile_name, Engine.get_frames_per_second(),
